...
class LedgerDetails extends React.Component {
    ...
    getLedgerTranxDetailsReport(reqData)
      .then((response) => {
        ...
        if (res.responseStatus === 200) {
          ...
          setTimeout(() => {
            if (this.state.ldview1 === 0) {
              document.getElementById("NewBTr_0")?.focus();
            } else if (this.state.ldview1 != null) {
              document.getElementById("NewBTr_" + this.state.ldview1)?.focus();
            }
          }, 500);
  ...
  getDrOpeningAmount = () => {
    ...
    if (crdrtype == "dr") {
      return isNaN(openingDramt)
        ? INRformat.format(0.0)
        : parseFloat(openingDramt).toFixed(2);
    } else {
      let openingDramt = 0;
      return openingDramt;
    }
  ...
  getCrOpeningAmount = () => {
    ...
    if (crdrtype == "cr") {
      return isNaN(openingCramt)
        ? INRformat.format(0.0)
        : parseFloat(openingCramt).toFixed(2);
    } else {
      let openingCramt = 0.0;
      return openingCramt;
    }
  };
  ...
  getDebitTotalAmount = () => {
    ...
    return isNaN(debitamt)
      ? INRformat.format(0.0)
      : parseFloat(debitamt).toFixed(2);
  };
  ...
  getCreditTotalAmount = () => {
    ...
    return isNaN(creditamt)
      ? INRformat.format(0.0)
      : parseFloat(creditamt).toFixed(2);
  };
  ...
  getClosingDrTotalAmount = () => {
    ...
    if (debitamt >= creditamt) {
      ...
      return isNaN(closing) ? 0 : Math.abs(parseFloat(closing)).toFixed(2);
    } else {
  ...
  componentDidMount() {
    if (AuthenticationCheck()) {
      ...
      console.log("prop_data in ledger details====>>>>>", prop_data.ldview);
      this.setState({ ldview1: prop_data.ldview });
      console.log("ldview --", this.state.ldview1)
      this.setState({
        edit_data: prop_data.prop_data,
        viewId: prop_data.viewId,
        selectedFilter: prop_data.selectedFilter,
        searchValue: prop_data.searchValue,
      });
      // console.log("prop_data.prop_data====",prop_data.prop_data);
      this.getlstLedgerTranxDetailsReports(prop_data.prop_data);
    ...
    return (
      <div className="ledger_form_style">
        <div className="cust_table">
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            ...
                            eventBus.dispatch("page_change", {
                              ...
                              to: "ledgerlist",
                              prop_data: {
                                viewId: this.props.block.prop_data.viewId,
                                selectedFilter: this.props.block.prop_data
                                  .selectedFilter,
                                searchValue: this.props.block.prop_data
                                  .searchValue,
                              },
                            });
          ...
          {/* {lstLedger.length > 0 && ( */}
          <div className="tbl-body-style-lV tbl-list-style1">
              ...
              <Table size="sm" className="tbl-font">
                ...
                <thead style={{ borderTop: "2px solid #dee2e6" }}>
                  <tr style={{ borderBottom: "1px solid transparent" }}>
                    ...
                    <td style={{ width: "7%" }}>
                      <b> {INRformat.format(this.getDrOpeningAmount())}</b>
                    </td>
                    <td style={{ width: "7%" }}>
                      <b> {INRformat.format(this.getCrOpeningAmount())}</b>
                    </td>
                    <td style={{ width: "7%" }}></td>
                  </tr>
                  <tr>
                    ...
                    <td style={{ width: "7%" }}>
                      <b> {INRformat.format(this.getDebitTotalAmount())}</b>
                    </td>
                    <td style={{ width: "7%" }}>
                      <b> {INRformat.format(this.getCreditTotalAmount())}</b>
                    </td>
                    <td style={{ width: "7%" }}></td>
                  </tr>
                  <tr>
                    ...
                    <td style={{ width: "7%" }}>
                      <b> {INRformat.format(this.getClosingDrTotalAmount())}</b>
                    </td>
                    <td style={{ width: "7%" }}>
                      <b>{INRformat.format(this.getClosingCrTotalAmount())}</b>
                    </td>
                    <td style={{ width: "7%" }}></td>
                  </tr>
                </thead>
