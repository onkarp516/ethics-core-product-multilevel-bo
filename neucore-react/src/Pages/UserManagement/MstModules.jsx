    ...
    getSysModuleTlst()
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          let data = res.list;
          if (data.length > 0) {
            this.setState({ lst_t_module: data, orgData: data });
          }
        }
      })
      .catch((error) => {});
  ...
  handleSearch = (vi) => {
    ...
    let orgData_F = orgData.filter(
      (v) =>
        (v.parent != null &&
          v.slug != null &&
          v.parent.toLowerCase().includes(vi.toLowerCase())) ||
        v.slug.toLowerCase().includes(vi.toLowerCase())
    );
    if (vi.length == 0) {
      this.setState({
        lst_t_module: orgData,
      });
    } else {
      this.setState({
        lst_t_module: orgData_F.length > 0 ? orgData_F : [],
      });
    }
  };
      ...
      <div className="company-form-style">
        <Collapse in={opendiv}>
          <div className="main-div mb-2 m-0">
            <h4 className="form-header">Create Module</h4>
            <Formik
              innerRef={this.myRef}
