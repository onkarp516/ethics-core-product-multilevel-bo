...
import { get_monthwise_sales_register_details } from "@/services/api_functions";
import {
  MyDatePicker,
  eventBus,
  AuthenticationCheck,
...
export default class SalesRegister1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      profitstep1: [],
  ...
  get_profit_and_loss_ac_details_on_date_change_of_user = () => {
    ...
    get_monthwise_sales_register_details(data)
      .then((response) => {
  ...
  get_profit_and_loss_ac_details_step1 = () => {
    get_monthwise_sales_register_details()
      .then((response) => {
        if (response.data.responseStatus === 200) {
          let result = response.data.response;
          console.log("result :", result);
