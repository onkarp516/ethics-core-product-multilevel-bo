...
import { get_cashbook_details } from "@/services/api_functions";
import {
  MyDatePicker,
  eventBus,
  AuthenticationCheck,
...
export default class CashOrBankBook1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      profitstep1: [],
    ...
    get_cashbook_details()
      .then((response) => {
        if (response.data.responseStatus === 200) {
          ...
          console.log("result :", result.cashParticular);
          let sdate = response.data.d_start_date;
          this.setState(
            {
              isEditDataSet: true,
              bankParticulars: result.bankParticular,
              cashParticulars: result.cashParticular,
              // profitstep1: result,
              startDate1: sdate,
      ...
      <div className="report-form-style">
        ...
        <div className="report-list-style-new">
          <Table className="m-0">
            ...
            >
              {cashParticulars.length > 0 ? (
                cashParticulars.map((v, i) => {
                  return (
                    <tr
              ...
              {bankParticulars.length > 0 ? (
                bankParticulars.map((v, i) => {
                  return (
                    <tr
                      style={{ border: "1px solid #DEE4EB" }}
                      onClick={(e) => {
