...
export default class Available1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      profitstep1: [],
    ...
    get_whole_stock_product()
      .then((response) => {
        ...
        if (res.responseStatus === 200) {
          let d = res.data.filter((v, i) => {
            let res_data = v.product_unit_data.filter(
              (v1) => v1.closing_stock > 0
            );
            v["product_stock"] = res_data;
            if (res_data.length > 0) return v;
          });

          console.warn("resulted d", d);
  ...
  getDebitTotalAmount = () => {
    ...
    let debitamt = 0;
    stocklst.map((v) => {
      let availablepath = v.product_unit_data;
      if (availablepath.length > 0) {
        availablepath.filter((k) => {
          if (k.qty > 0) {
        ...
        <div className="report-list-style">
          <Table className="m-0">
            ...
            <tbody className="tbody_style">
              {stocklst.length > 0 ? (
                stocklst.map((v, i) => {
                  if (v.product_stock && v.product_stock.length > 0) {
                    return (
                      <tr
