...
export default class WholeStock3 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      profitstep1: [],
  ...
  PreviuosPageProfit = (status) => {
    const { product_name, productId } = this.state;
    // let { lerdgerCreate } = this.setState;
    eventBus.dispatch("page_change", {
      from: "wholeStock3",
      to: "wholeStock2",
  ...
  componentDidMount() {
    if (AuthenticationCheck()) {
      ...
      // this.getlstLedger();
      if (prop_data != "") {
        this.setState({ productId: prop_data.productId });
        this.setState({ product_name: prop_data.product_name });
        this.setState({ closing_stock: prop_data.closing_stock });
  ...
  get_profit_and_loss_ac_details_on_date_change_of_user = (vp) => {
    let { edit_data, startDate, endDate } = this.state;
    console.log("edit_data--", vp);
    let startDatecon = "";
    let endDatecon = "";
    if (startDate != "") {
      startDatecon = moment(startDate).format("YYYY-MM-DD");
      data.append("start_date", startDatecon);
    }
    if (endDate != "") {
      endDatecon = moment(endDate).format("YYYY-MM-DD");
      data.append("end_date", endDatecon);
    }
    let data = new FormData();

    data.append("productId", vp);
    data.append("product_name", vp);
    data.append("closing_stock", vp);
