...
class CmpTRow extends Component {
    ...
    setRowProductData = () => {
        ...
        console.log("batchDetailsId", batchDetailsId);
        ...
        if (productLst.length > 0) {
            rows = rows.map((v, i) => {
                ...
                if (selectedProduct != "") {
                    ...
                    let igst = selectedProduct ? selectedProduct.igst : 0;
                    console.log("igst", igst);
                    console.log("v----", v);
                    if (igst > 0 && "org_rate" in v) {
                        // let reverseCal = (parseFloat(v.org_rate) * parseFloat(igst)) / (100 + parseFloat(igst));
                        // console.log("reverseCal", reverseCal, (v.org_rate));
                        // let finalRate = (parseInt(v.qty) * parseFloat(v.org_rate)) - (parseInt(v.qty) * reverseCal);
                        // console.log("finalRate", finalRate)
                        // v["rate"] = finalRate.toFixed(2);
                        let disAmt = (parseFloat(v.org_rate) * parseFloat(v.dis_per)) / 100;
                        console.log("disAmtPerQty", disAmt);

                        let perTotal = parseFloat(v.org_rate) - disAmt;
                        console.log("perTotal", perTotal);

                        // let disAmtTotal = (parseInt(v.qty) * parseFloat(v.org_rate)) * (parseFloat(v.dis_per) / 100);
                        let disAmtTotal = parseFloat(v.qty) * disAmt;
                        console.log("disAmtTotal", disAmtTotal);
                        let disPerRate = parseFloat(v.org_rate) - disAmt;
                        console.log("disPerRate", disPerRate);
                        let disRatePerTotal = disPerRate * parseInt(v.qty);
                        console.log("disRatePerTotal", disRatePerTotal);
                        let gstPerqty =
                            (disPerRate * parseFloat(igst)) / (100 + parseFloat(igst));
                        console.log("gstPerqty", gstPerqty);
                        let taxableAmt = disRatePerTotal - gstPerqty;
                        console.log("taxableAmt", taxableAmt);
                        let gstAmt = gstPerqty * parseInt(v.qty);
                        console.log("gstAmt", gstAmt);

                        let netAmt = taxableAmt + gstAmt;
                        console.log("netAmt", netAmt);
                        v["rate"] = taxableAmt.toFixed(2);
                        // v["base_amt"] = disRatePerTotal && disRatePerTotal != "" ? disRatePerTotal : 0;

                        // v["dis_amt"] = disAmt;
                        // v["dis_per"] = v.dis_per;
                        // v["dis_per_cal"] = 0;
                        // v["dis_amt_cal"] = 0;
                        // v["total_amt"] = disRatePerTotal != "" ? disRatePerTotal : 0;
                        // v["total_base_amt"] = 0;
                        // v["gst"] = 0;
                        // v["igst"] = 0;
                        // v["cgst"] = 0;
                        ...
                        // v["total_cgst"] = 0;
                        // v["total_sgst"] = 0;
                        // v["final_amt"] = netAmt && netAmt != "" ? netAmt : 0;

                        // v["gross_amt"] = 0;
                        // v["add_chg_amt"] = 0;
                        // v["gross_amt1"] = 0;
                        // v["invoice_dis_amt"] = 0;
                        // v["net_amt"] = netAmt && netAmt != "" ? netAmt : 0;
                        // v["taxable_amt"] = taxableAmt != "" ? taxableAmt : 0;
                    } else {
                        if (
    ...
    handleProductTableRow(event) {
        ...
        let { code, currentSelectedProductId } = this.state;
        ...
        //   let searchInput = code + event.key;

        //   this.setProductInputData(searchInput, false, currentSelectedProductId);

        //   console.warn("selectedProduct->>>>>>", currentSelectedProductId);
        //   let { productLst, orgProductLst } = this.state;
        //   if (searchInput != "") {
        //     let filterData = [];
        //     orgProductLst != "" &&
        ...
        //           (v.packing != null &&
        //             v.packing.toLowerCase().includes(searchInput.toLowerCase()))
        //         ) {
        //           filterData.push(v);
        //         }
        //       });
    ...
    handleBatchTableRow(event) {
        ...
        if (k === 40) {
            ...
            console.warn("Is expired->>>>>>", NoOfDays);
            let isConfirm = "";
            if (NoOfDays < 0) {
                MyNotifications.fire({
                    show: true,
        ...
        //     batchDataList != "" &&
        //       batchDataList.map((v) => {
        //         if (v.batch_no.includes(v.test.trim())) {
        //           filterData.push(v);
        //         }
      ...
      /> */}
                ...
                <div class="outer-wrapper">
                    <div class="table-wrapper">
                        <div
                            className={`${transactionTableStyle == "salesOrder" ||
                                    ...
                                    <tr>
                                        ...
                                        {isBatchOpen == false ? (
                                            <>
                                                ...
                                                {ABC_flag_value == "ABC" ? (
                                                    <th
                                                        className={`${ABC_flag_value == "ABC" ? "Level_ABC" : "Level_no"
                                                            }`}
                                                    >
                                    ...
                                    {rows.map((rv, ri) => {
                                        return (
                                            ...
                                            >
                                                ...
                                                <td>
                                                    <Form.Group
                                                        className={`${unitIdData && unitIdData[ri] == "Y"
                                                            ...
                                                            <Form.Control
                                                                ...
                                                                onClick={(e) => {
                                                                    ...
                                                                    if (rv.selectedProduct?.is_serial === true) {
                                                                        ...
                                                                        // if (rows[ri]["productName"]) {
                                                                        //   // alert("hii");
                                                                        // }
                                                                        if (rv.selectedProduct.is_batch == true) {
                                                                ...
                                                                // @prathmesh @Without Product selection batch popup open end
                                                ...
                                                <td>
                                                    <Form.Control
                                                        ...
                                                        placeholder="0"
                                                        onChange={(e) => {
                                                            handleUnitChange("org_rate", e.target.value, ri);
                                                        }}
                                                ...
                                                <td>
                                                    <Form.Control
                                                        ...
                                                        value={this.getFloatUnitElement("org_base_amt", ri)}
                                                        disabled
                                                        readOnly
                                                    />
                                                </td>
                                                                            ...
                                                                            onKeyDown={(e) => {
                                                                                if (e.shiftKey && e.key === "Tab") {
                                                                                    ...
                                                                                    if (
                                                                                        ...
                                                                                        // this.focusNextElement(e)
                                                                                        document.getElementById("b_expiry").focus();
                                                                                    }
                                                                                }
                                                                            }}
                                                                            onBlur={(e) => {
