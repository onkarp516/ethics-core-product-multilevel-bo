...
import {
  ...
  createSalesInvoice,
  removeInstance,
  get_counter_sales_product_fpu_by_ids,
  get_counter_sales_by_no,
...
import CounterCmpTRow from "@/Pages/Tranx/Sales/CounterSale/CounterCmpTRow";
...
class TranxSalesInvoiceEdit extends React.Component {
  constructor(props) {
    ...
    this.saleAccRef = React.createRef();
  ...
  getSalesOrderProductFpuByIds = () => {
    ...
    let sales_order_ids = salesEditData.modeCheck.map((v) => {
      return { id: v };
    });
    console.log("cs_ids", sales_order_ids);

    reqData.append("cs_ids", JSON.stringify(sales_order_ids));

    get_counter_sales_product_fpu_by_ids(reqData)
      .then((res) => res.data)
      .then((response) => {
  ...
  callCreateInvoice = () => {
    ...
    console.log(
      "callCreateInvoice CreateInvoice ",
      values,
      this.state.salesEditData.modeCheck
    );
    ...
    requestData.append(
      "bill_dt",
      moment(new Date(moment(new Date(), "DD/MM/YYYY").toDate())).format(
        "YYYY-MM-DD"
      )
    );
    requestData.append("bill_no", invoice_data.bill_no);
    requestData.append("sales_acc_id", 1);
    ...
    if (this.state.salesEditData.modeCheck.length > 0) {
      requestData.append(
        "reference_cs_ids",
        this.state.salesEditData.modeCheck.join(",")
      );
    }
    requestData.append("reference_type", "CS");
    requestData.append(
      "sales_discount",
    ...
    if (
      ...
      requestData.append(
        "addChgLedgerAmt1",
        values.additionalChgLedgerAmt1 && values.additionalChgLedgerAmt1 !== ""
          ? values.additionalChgLedgerAmt1
          : 0
      );
    }
    if (
      ...
      requestData.append(
        "addChgLedgerAmt2",
        values.additionalChgLedgerAmt2 && values.additionalChgLedgerAmt2 !== ""
          ? values.additionalChgLedgerAmt2
          : 0
      );
    }
    if (
      ...
      requestData.append(
        "addChgLedgerAmt3",
        values.additionalChgLedgerAmt3 && values.additionalChgLedgerAmt3 !== ""
          ? values.additionalChgLedgerAmt3
          : 0
      );
    }
          ...
          MyNotifications.fire(
            {
              ...
              handleFailFn: () => {
                ...
                if ("ledgerId" in this.state.source != "") {
                  eventBus.dispatch("page_change", {
                    from: "tranx_countersale_to_saleinvoice_gst",
                    to: "ledgerdetails",
                    // prop_data: {
  ...
  setSalesInvoiceEditData = () => {
    console.log("HHHHH");
    const { salesEditData, initVal } = this.state;

    let reqData = new FormData();
    console.log("salesEditData=-> ", salesEditData, initVal);
    let sales_order_ids = salesEditData.modeCheck.map((v) => {
      return { id: v };
    });
    console.log("csno", sales_order_ids);

    reqData.append("csno", JSON.stringify(sales_order_ids));
    get_counter_sales_by_no(reqData)
      .then((response) => {
        let res = response.data;
        ...
        if (res.responseStatus === 200) {
          ...
          if (row.length > 0) {
            ...
            initRowData = row.map((v, i) => {
              ...
              // Counter Sales Calculation
              // console.log("v.rate", v.rate);
              // console.log("v.igst", v.igst);
              // let reverseCal = (parseFloat(v.rate) * parseFloat(v.igst)) / (100 + parseFloat(v.igst));
              // console.log("reverseCal", reverseCal)
              // let finalRate = (parseInt(v.qty) * parseFloat(v.rate)) - reverseCal;
              ...
              v["reference_id"] = v.counterId != "" ? v.counterId : "";
              v["reference_type"] = "CS";
  ...
  handleTranxCalculation = (elementFrom = "", isCal = false) => {
    ...
    let resTranxFn = fnTranxCalculationGST({
  ...
  getProductBatchList = (rowIndex, source = "batch") => {
    ...
    if (document.getElementById("CSIGAddBtn-" + id) != null) {
      setTimeout(() => {
        document.getElementById("CSIGAddBtn-" + id).focus();
      });
    }
  };
  handleMstState = (rows) => {
    this.setState({ rows: rows }, () => {
      ...
      setTimeout(() => {
        document.getElementById("CSIGProductId-" + id).focus();
      }, 1000);
    });
  };
  ...
  productModalStateChange = (obj, callTrxCal = false) => {
    ...
    if (obj.isProduct == "productMdl") {
      console.log("first", obj.isProduct);
      setTimeout(() => {
        document
          .getElementById("CSIGProductId-particularsname-" + obj.rowIndex)
          .focus();
      }, 1000);
    }
    ...
    if (obj.isBatchMdl == "batchMdl") {
      setTimeout(() => {
        document
          .getElementById("CSIGProductId-batchNo-" + obj.rowIndex)
          .focus();
      }, 1000);
    }
  };
      ...
      <>
        ...
        <div className="purchase-tranx" style={{ overflow: "hidden" }}>
                              ...
                              <Form.Control
                                ...
                                onKeyDown={(e) => {
                                  if (e.keyCode == 13) {
                                    // this.focusNextElement(e);
                                    document
                                      .getElementById("TSIC_submit_btn")
                                      .focus();
                                  }
                                }}
                              />
                        ...
                        <Button
                          ...
                          onKeyDown={(e) => {
                            if (e.keyCode === 32) {
                              ...
                              // this.myRef.current.handleSubmit();
                              document.getElementById("CSIGcancel").focus();
                            }
                          }}
                        >
                          Submit
          ...
          <MdlLedger
            ...
            ledgerType={"SD"} // @prathmesh @ledger filter added
          />
          {/* Ledger Modal Ends */}
