...
class TranxSalesOrderCreate extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      ledgerId: "",
      setLedgerId: false,
      from_source: "tranx_sales_order_create",
    ...
    getLastSalesOrder()
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          ...
          // }
          let { invoice_data } = this.state;
          invoice_data["so_sr_no"] = res.count;
          invoice_data["so_bill_no"] = res.serialNo;
  ...
  handleMstState = (rows) => {
    this.setState({ rows: rows }, () => {
      ...
      setTimeout(() => {
        document.getElementById("TSOCProductId-particularsname-" + id).focus();
      }, 1000);
    });
  };
  ...
  productCreate = (e = null) => {
    ...
    eventBus.dispatch("page_change", {
      ...
      to: "newproductcreate",
      isNewTab: true,
      prop_data: { tran_no: TRAN_NO.prd_tranx_sales_order_create },
    });
  };
  ...
  getProductBatchList = (rowIndex = -1) => {
    ...
    // if (document.getElementById("TSOCAddBtn-" + id) != null) {
    //   setTimeout(() => {
    //     document.getElementById("TSOCAddBtn-" + id).focus();
    //   });
    // }
  };
  ...
  callCreatesaleOrder = () => {
    ...
    // debugger;
    // //console.log("sales_acc_id", invoice_data.salesAccId.value)
    if (invoice_data.salesAccId != undefined)
            ...
            () => {
              ...
              setTimeout(() => {
                ...
                if (prop_data.isProduct === "productMdl") {
                  document
                    .getElementById(
                      "TSOCProductId-particularsname-" + prop_data.rowIndex
                    )
                    .focus();
                } else {
                  this.inputLedgerNameRef.current.focus();
  ...
  productModalStateChange = (obj, callTrxCal = false) => {
    ...
    if (obj.isProduct == "productMdl") {
      setTimeout(() => {
        document
          .getElementById("TSOCProductId-particularsname-" + obj.rowIndex)
          .focus();
      }, 250);
    ...
    if (obj.isBatchMdl == "batchMdl") {
      setTimeout(() => {
        document
          .getElementById("TSOCProductId-batchNo-" + obj.rowIndex)
          .focus();
      }, 250);
    }
  };
    ...
    getValidatePurchaseInvoice(reqData)
      .then((response) => {
        ...
        if (res.responseStatus == 409) {
          ...
          this.orderDate.current?.focus();
        }
      })
      .catch((error) => {});
  };

  //start of validatio of order number
  // validate_sales_orderFun = (salesInvoiceNo = "", setFieldValue) => {
  //   let { invoice_data } = this.state;
  //   let requestData = new FormData();
  //   requestData.append("salesOrderNo", salesInvoiceNo);
  //   validate_sales_OrderNo(requestData)
  //     .then((response) => {
  //       console.log("res", response);
  //       let res = response.data;
  //       if (res.responseStatus != 200) {
  ...
  //         setFieldValue("so_bill_no", invoice_data.so_bill_no);
  //         document.getElementById("so_bill_no").focus();
  //       } else {
  //         this.orderNumber.current.focus();
  //       }
  //     })
  //     .catch((error) => {
  //       console.log("error", error);
  //     });
  // };
  validate_sales_orderFun = (invoice_no) => {
    let reqData = new FormData();
    reqData.append("salesOrderNo", invoice_no);
    validate_sales_OrderNo(reqData)
      .then((response) => {
        let res = response.data;
        if (res.responseStatus == 409) {
          MyNotifications.fire({
  ...
  //end of validation of order number
  ...
  //         //       eventBus.dispatch("page_change", "tranx_sales_order_create");
  //         //       // this.reloadPage();
  //         //     },
  //         //   },
  //         //   () => { }
  ...
  handlePurchaseBillsSelectionAll = (status) => {
    ...
    let fBills = this.state.salesPendingQuatLst;
    let lstSelected = [];
    if (status == true) {
      lstSelected = this.state.salesPendingQuatLst.map((v) => v.id);

      console.log("lst", lstSelected);
    }
    this.setState({
      isAllChecked: status,
      penQuatSelectedLst: lstSelected,
      salesPendingQuatLst: fBills,
    });
  };
                              ...
                              <Col lg={4} md={4} sm={4} xs={4}>
                                <Form.Group
                                  onKeyDown={(e) => {
                                    ...
                                    // else if(e.keyCode==13)this.focusNextElement(e)
                                  }}
                                >
                                  <MyTextDatePicker
                                    ref={this.orderDate}
                                  ...
                                  onBlur={(e) => {
                                    ...
                                    if (
                                      ...
                                      if (
                                        ...
                                        console.log("tranx_date", order_Date);
                                        if (curdate >= order_Date) {
                                          setFieldValue(
                                            "bill_dt",
                                            e.target.value
                                          );
                                          ...
                                          MyNotifications.fire({
                                            ...
                                            icon: "error",
                                            title: "Error",
                                            msg: "Order date should not be Greater than today's date",
                                            is_button_show: true,
                                          });
                                          setFieldValue("bill_dt", "");
                                        }
                              ...
                              <Button
                                ...
                                onKeyDown={(e) => {
                                  if (e.shiftKey && e.keyCode == 9) {
                                    ...
                                    document
                                      .getElementById("TSOC_Ledger_Tab")
                                      .focus();
                                    this.setState({ currentTab: "first" });
                                  }
                                }}
