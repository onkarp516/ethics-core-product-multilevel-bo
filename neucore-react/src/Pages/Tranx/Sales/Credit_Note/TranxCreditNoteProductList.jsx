...
class TranxCreditNoteProductList extends React.Component {
  constructor(props) {
    ...
    this.clientGSTRef = React.createRef();
    this.salesAcRef = React.createRef();

    this.state = {
      disabledType: "salesReturnCreate",
      ...
      from_source: "tranx_credit_note_product_list",
      sourceUnder: "sale",
      ledgerInputData: "",
      errorArrayBorder: "",
  ...
  handleMstState = (rows) => {
    this.setState({ rows: rows }, () => {
      ...
      setTimeout(() => {
        document.getElementById("TSRCProductId-particularsname-" + id).focus();
      }, 1000);
    });
  };
  ...
  validate_sales_invoicesFun = (salesInvoiceNo = "", setFieldValue) => {
    ...
    validate_sales_return(requestData)
      .then((response) => {
        //console.log("res", response);
        let res = response.data;
        if (res.responseStatus != 200) {
            ...
            () => {
              ...
              setTimeout(() => {
                ...
                if (prop_data.isProduct === "productMdl") {
                  document
                    .getElementById(
                      "TSRCProductId-particularsname-" + prop_data.rowIndex
                    )
                    .focus();
                } else {
                  this.inputLedgerNameRef.current.focus();
  ...
  ledgerModalStateChange = (ele, val) => {
    if (ele == "ledgerModal" && val == true) {
      ...
      this.setState({ [ele]: val }, () => {
        // this.inputLedgerNameRef.current.focus();
        if (this.state.lstGst.length > 1 && this.clientGSTRef.current) {
          this.clientGSTRef.current.focus();
        } else if (this.salesAcRef.current) {
          this.salesAcRef.current.focus();
        }
      });
    } else {
      this.setState({ [ele]: val });
  ...
  callCreateInvoice = () => {
    ...
    requestData.append("sales_invoice_id", selectedBillNo.id);
    requestData.append("source", selectedBillNo.source);

    requestData.append("debtors_id", invoiceValues.selectedSupplier.id);
    // !Invoice Data
  ...
  getProductBatchList = (rowIndex = -1, source = "batch") => {
    ...
    // if (document.getElementById("TSRCAddBtn-" + id) != null) {    //commented due to cursor should not go on + icon
    //   setTimeout(() => {
    //     document.getElementById("TSRCAddBtn-" + id).focus();
    //   });
    // }
  };
  ...
  productModalStateChange = (obj, callTrxCal = false) => {
    ...
    if (obj.isProduct == "productMdl") {
      setTimeout(() => {
        document
          .getElementById("TSRCProductId-particularsname-" + obj.rowIndex)
          .focus();
      }, 250);
    ...
    if (obj.isBatchMdl == "batchMdl") {
      setTimeout(() => {
        // document
        //   .getElementById("TSRCProductId-batchNo-" + obj.rowIndex)
        //   .focus();
      }, 250);
    }

    if (obj.rowProductModal == false) {
      setTimeout(() => {
        this.salesAcRef.current.focus();
      }, 1000);
    }
    ...
    if (row.length > 0) {
      initRowData = row.map((v, i) => {
        ...
        v["unitId"] = v.unitId
          ...
          : "";
        v["unit_id"] = unit_id;
        v["qty"] = v.returnable_qty != "" ? v.returnable_qty : "";
    ...
    f_billLst = f_billLst.map((v, i) => {
      if (remTotalDebitAmt > 0) {
        if (f_selectedBills.includes(v.invoice_id)) {
          ...
          console.log("remaining_amt", v["remaining_amt"]);
        ...
        <div className="purchase-tranx" style={{ overflow: "hidden" }}>
          <Formik
            ...
            onSubmit={(values, { setSubmitting, resetForm }) => {
              ...
              if (values.bill_no.trim() == "") {
                errorArray.push("Y");
              } else {
                errorArray.push("");
              }
              if (values.selectedSupplier.balancingMethod == "bill-by-bill") {
                if (values.mode == "") {
                  errorArray.push("Y");
                } else {
                  errorArray.push("");
