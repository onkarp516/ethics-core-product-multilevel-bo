...
class TranxCreditNoteEdit extends React.Component {
  constructor(props) {
    super(props);
  ...
  getProductFlavorpackageUnitbyids = () => {
    ...
    if (this.state.salesReturnData.id != undefined) {
      reqData.append("id", this.state.salesReturnData.id);
    } else {
  ...
  handleRowClick = (v) => {
    ...
    // ;
    let prop_data = {
      returnIntiVal: this.myRef.current.values,
      returnData: v,
      invoice_data: v,
    };

    this.setState(
      // { invoiceedit: false, salesReturnData: prop_data, show: false },
      { salesReturnData: prop_data, show: false },
  ...
  setSalesInvoiceEditData = () => {
    const { id } = this.state.salesReturnData;
    ...
    getSalesReturnById(formData)
      .then((response) => {
        ...
        if (res.responseStatus === 200) {
          ...
          let initInvoiceData = {
            id: invoice_data.id,
            credit_note_sr: invoice_data.sales_sr_no,
            sales_return_invoice: invoice_data.invoice_no,
            invoice_dt:
            ...
            clientCodeId: invoice_data.debtor_id
          ...
          console.warn("rahul::initInvoiceData ", initRowData, initInvoiceData);
          this.setState(
            {
  ...
  componentDidMount() {
    if (AuthenticationCheck()) {
      ...
      this.setState({ source: prop_data, salesReturnData: prop_data }, () => {
  ...
  componentDidUpdate() {
    ...
    console.log("sale rtyen", salesReturnData);
...
                    </Col> */}
              ...
              <div className="purchasescreen">
                <Formik
                  ...
                  onSubmit={(values, { setSubmitting, resetForm }) => {
                    ...
                    this.handleSubmitSCList(values.clientNameId.value);
                  }}
                >
                  {({
                    values,
                        ...
                        <Col md="3">
                          <Form.Group>
                            ...
                            <Form.Control
                              ...
                              value={
                                values.clientNameId &&
                                values.clientNameId.ledger_balance +
                                  " " +
                                  values.clientNameId.ledger_balance_type
                              }
                              readOnly
                            />
          ...
          >
            ...
            <Formik
              ...
              onSubmit={(values, { setSubmitting, resetForm }) => {
                console.log("values", values);
                let { initVal, salesReturnData } = this.state;
                ...
                requestData.append("transaction_dt", values.transaction_dt);
                requestData.append(
                  "debtors_id",
                  invoiceValues.clientCodeId.value
                );
                // !Invoice Data
                console.log("returnValues ", invoiceValues);
                ...
                if (is_expired != true) {
                  if (b_details_id != 0) {
                    rows[rowIndex]["brandDetails"][brandIndex][
                      ...
                      "unitDetails"
                    ][unitIndex]["rate"] = b_details_id.min_rate_a;
                    rows[rowIndex]["brandDetails"][brandIndex][
                      "categoryDetails"
                    ][categoryIndex]["subcategoryDetails"][subcategoryIndex][
                      "flavourDetails"
                        ...
                        let data = {
                          ...
                          invoice_data:
                            this.myRef != null && this.myRef.current
                              ? this.myRef.current.values
                              : "",
                          from_page: "tranx_credit_note_edit",
                        };
                        eventBus.dispatch("page_change", {
                          from: "tranx_credit_note_edit",
                          to: "ledgercreate",
...
export default connect(mapStateToProps, mapActionsToProps)(TranxCreditNoteEdit);
