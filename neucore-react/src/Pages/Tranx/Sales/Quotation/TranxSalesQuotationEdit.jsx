...
import {
  ...
  getSalesQuotationProductFpuById,
  getSundryDebtors,
  ...
  validate_sales_quotation_update,
...
class TranxSalesQuotationEdit extends Component {
  constructor(props) {
    super(props);
    ...
    this.selectClientGSTINRef = React.createRef();
    ...
    this.state = {
      ...
      ledgerId: "",
      setLedgerId: false,
      from_source: "tranx_sales_quotation_edit",
  ...
  // Flavour Package Unit

  getSalesQuotationProductFpuByIds = (invoice_id) => {
    let reqData = new FormData();
    reqData.append("id", invoice_id);
    getSalesQuotationProductFpuById(reqData)
      .then((res) => res.data)
      .then((response) => {
    ...
    getSundryDebtors()
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          ...
          //console.log("opt, codeopt>>>", opt, codeopt);
          this.setState({
            supplierNameLst: opt,
  ...
  handleMstState = (rows) => {
    this.setState({ rows: rows }, () => {
      ...
      setTimeout(() => {
        document.getElementById("TSQEProductId-particularsname-" + id).focus();
      }, 1000);
    });
  };
  ...
  ledgerCreate = () => {
    ...
    eventBus.dispatch("page_change", {
      from: "tranx_sales_quotation_edit",
      to: "ledgercreate",
  ...
  productCreate = (e = null) => {
    ...
    eventBus.dispatch("page_change", {
      ...
      to: "newproductcreate",
      isNewTab: true,
      prop_data: { tran_no: TRAN_NO.prd_tranx_sales_quotation_edit },
    });
  };
  ...
  callCreatesaleQuatation = () => {
    ...
    if (invoice_data.selectedSupplier && invoice_data.selectedSupplier != "") {
      // this.myRef.current.setFieldValue(
      //   "supplierCodeId",
      ...
      opt = invoice_data.selectedSupplier.gstDetails.map((v, i) => {
        return {
          label: v.gstin,
          value: v.id,
        };
    ...
    updateSalesQuotation(requestData)
      .then((response) => {
        let res = response.data;
        if (res.responseStatus == 200) {
          // MyNotifications.fire({
            ...
            () => {
              ...
              setTimeout(() => {
                ...
                if (prop_data.isProduct == "productMdl") {
                  document
                    .getElementById(
                      "TSQEProductId-particularsname-" + prop_data.rowIndex
                    )
                    .focus();
      ...
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          ...
          let initInvoiceData = {
            ...
            bill_no: invoice_data.sq_bill_no,
            gstNo: invoice_data.gstNo,
            bill_dt:
              invoice_data.bill_date != ""
                ? moment(
                    new Date(
                      moment(invoice_data.bill_date, "YYYY-MM-DD").toDate()
                    )
                  ).format("DD/MM/YYYY")
                : "",
            transaction_dt:
  ...
  componentDidMount() {
    if (AuthenticationCheck()) {
      ...
      this.setState({ salesEditData: prop_data.prop_data }, () => {
        if (prop_data.prop_data.id) {
          this.getSalesQuotationProductFpuByIds(prop_data.prop_data.id);
        }
      });
  ...
  validate_sales_quotationFun = (invoice_no, quotation_id) => {
    const { id } = this.state.salesEditData;
    let reqData = new FormData();
    reqData.append("invoice_id", id);
    reqData.append("salesQuotationNo", invoice_no);
    validate_sales_quotation_update(reqData)
      .then((response) => {
        let res = response.data;
        if (res.responseStatus == 409) {
          MyNotifications.fire({
    ...
    if (ele == "ledgerModal" && val == true) {
      ...
      this.setState({ [ele]: val }, () => {
        ...
        if (this.state.ledgerNameFlag) {
          if (
            this.state.lstGst.length > 1 &&
            this.selectClientGSTINRef.current
          ) {
            this.selectClientGSTINRef.current.focus();
  ...
  productModalStateChange = (obj, callTrxCal = false) => {
    ...
    if (obj.isProduct == "productMdl") {
      setTimeout(() => {
        document
          .getElementById("TSQEProductId-particularsname-" + obj.rowIndex)
          .focus();
      }, 250);
    ...
    if (obj.isBatchMdl == "batchMdl") {
      setTimeout(() => {
        document
          .getElementById("TSQEProductId-batchNo-" + obj.rowIndex)
          .focus();
      }, 250);
    }
  };
                                ...
                                <Form.Control
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.shiftKey && e.key === "Tab") {
                                      ...
                                      if (e.target.value) {
                                        e.preventDefault();
                                        this.validate_sales_quotationFun(
                                          values.bill_no,
                                          values.EditsupplierId,
                                          setFieldValue
                                        );
                                        // this.focusNextElement(e);
                                      }
                              ...
                              <Button
                                ...
                                onKeyDown={(e) => {
                                  if (e.shiftKey && e.keyCode == 9) {
                                    ...
                                    document
                                      .getElementById("TSQE_Ledger_Tab")
                                      .focus();
                                    this.setState({ currentTab: "first" });
                                  }
                                }}
