...
class TranxSalesQuotationCreate extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      ledgerId: "",
      setLedgerId: false,
      from_source: "tranx_sales_quotation_create",
  ...
  handleMstState = (rows) => {
    this.setState({ rows: rows }, () => {
      ...
      setTimeout(() => {
        document.getElementById("TSQCProductId-particularsname-" + id).focus();
      }, 1000);
    });
  };
            ...
            () => {
              ...
              setTimeout(() => {
                ...
                if (prop_data.isProduct === "productMdl") {
                  document
                    .getElementById(
                      "TSQCProductId-particularsname-" + prop_data.rowIndex
                    )
                    .focus();
                } else {
                  this.inputLedgerNameRef.current.focus();
  ...
  getInvoiceBillsLstPrint = (bill_no) => {
    //console.log("id Invoice----", id);
    let reqData = new FormData();
    reqData.append("id", bill_no);
  ...
  validate_sales_quotationFun = (invoice_no) => {
    let reqData = new FormData();
    reqData.append("salesQuotationNo", invoice_no);
    validate_sales_quotationNo(reqData)
      .then((response) => {
        let res = response.data;
        if (res.responseStatus == 409) {
          MyNotifications.fire({
  ...
  ledgerModalStateChange = (ele, val) => {
    if (ele == "ledgerModal" && val == true) {
      ...
      this.setState({ [ele]: val }, () => {
        // this.inputLedgerNameRef.current.focus();
        if (this.state.lstGst.length > 1 && this.selectClientGSTINRef.current) {
  ...
  productModalStateChange = (obj, callTrxCal = false) => {
    ...
    if (obj.isProduct == "productMdl") {
      setTimeout(() => {
        document
          .getElementById("TSQCProductId-particularsname-" + obj.rowIndex)
          .focus();
      }, 250);
    ...
    if (obj.isBatchMdl == "batchMdl") {
      setTimeout(() => {
        document
          .getElementById("TSQCProductId-batchNo-" + obj.rowIndex)
          .focus();
      }, 250);
    }
  };
                              ...
                              <Button
                                ...
                                onKeyDown={(e) => {
                                  if (e.shiftKey && e.keyCode == 9) {
                                    ...
                                    document
                                      .getElementById("TSQC_Ledger_Tab")
                                      .focus();
                                    this.setState({ currentTab: "first" });
                                  }
                                }}
