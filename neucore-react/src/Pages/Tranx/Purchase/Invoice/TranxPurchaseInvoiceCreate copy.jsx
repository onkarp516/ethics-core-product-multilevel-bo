...
class TranxPurchaseInvoiceCreate extends Component {
          ...
          MyNotifications.fire(
            {
              show: true,
              icon: "warning",
              title: "Invoice date not valid as per Fiscal year",
  ...
  getLedgerByCode(code) {
    ...
    let { ledgerList, invoice_data } = this.state;
    // console.warn("ledgerList->>>>>>>>>>", ledgerList);

    let ledgerData = ledgerList.find(
    ...
    if (ledgerData) {
      if (this.myRef.current) {
        ...
        this.myRef.current.setFieldValue("selectedSupplier", ledgerData);
        let opt =
          ledgerData && ledgerData.gstDetails
            ? ledgerData.gstDetails.map((vi) => {
        ...
        this.setState(
          {
            ...
            gstId: getSelectValue(opt, ledgerData.gstDetails[0].id),
          },
          () => {
            ...
            if (opt.length > 0 && ledgerData) {
              this.myRef.current.setFieldValue(
                "gstId",
                getSelectValue(opt, ledgerData.gstDetails[0].id)
              );
            }
          }
        );
        ...
        >
          <Formik
            ...
            onSubmit={(values, { setSubmitting, resetForm }) => {
              console.log("values_________", values);
                      ...
                      <Row>
                        ...
                        <Col lg={6} className="my-auto">
                          ...
                          <span className="span-value">
                            ...
                            {isNaN(INRformat.format(product_hover_details.mrp))}
                          </span>
                          <span className="custom_hr_style">|</span>
                        </Col>
                      </Row>
