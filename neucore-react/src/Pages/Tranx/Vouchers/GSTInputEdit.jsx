...
export default class GSTInputEdit extends Component {
  constructor(props) {
    super(props);
  ...
  getSelectListValue = (opts, val) => {
    return opts.find((o) => o.id === val);
  };

  setGSTInputEditData = () => {
    const { id } = this.state.gstEditData;
    let { ledgerList, voucherList, optHSNList, optTaxList, paymentOpt } =
      this.state;
    let formData = new FormData();
    formData.append("id", id);
    getGstInputById(formData).then((response) => {
      let res = response.data;
      if (res.responseStatus == 200) {
        console.log("getGstInputById--->", res);
        let {
          voucher_no,
          row,
          voucher_sr_no,
        ...
        this.myRef.current.setFieldValue("id", res.id);
        this.myRef.current.setFieldValue("voucher_no", voucher_no);
        this.myRef.current.setFieldValue("voucher_sr_no", voucher_sr_no);
        this.myRef.current.setFieldValue("narration", narration);
        this.myRef.current.setFieldValue("transaction_dt", transaction_dt);

        // this.myRef.current.setFieldValue("grandTotal", grand_total);
        ...
        this.myRef.current.setFieldValue("partyNameId", partyNameId);

        this.myRef.current.setFieldValue(
          "partyName",
          partyNameId
            ? this.getSelectListValue(ledgerList, partyNameId)["ledger_name"]
            : ""
        );
        this.myRef.current.setFieldValue("postingAccId", postingAccId);
        this.myRef.current.setFieldValue(
          "postingAcc",
          postingAccId
            ? this.getSelectListValue(voucherList, postingAccId)["ledger_name"]
            : ""
        );
        this.myRef.current.setFieldValue(
          "payment_mode",
          pay_mode ? getSelectValue(paymentOpt, parseInt(pay_mode)) : ""
        );
        this.myRef.current.setFieldValue(
          "pay_mode",
          pay_mode ? getSelectValue(paymentOpt, parseInt(pay_mode)).value : ""
        );
        this.myRef.current.setFieldValue(
          "voucher_dt",
          moment(voucher_dt).format("DD/MM/YYYY")
        );
        // this.myRef.current.setFieldValue("round_off_amt", round_off_amt);
        ...
        if (row.length > 0) {
          initRowData = row.map((v, i) => {
            ...
            v["productId"] = v.details_id ? v.details_id : "";
            v["details_id"] = v.details_id != "" ? v.details_id : 0;
            // v["hsn"] = v.hsn_selected != "" ? v.hsn_selected : 0;
            v["hsn"] = v.hsn_selected
              ? getSelectValue(optHSNList, parseInt(v.hsn_selected))
              : "";
            v["tax"] = v.tax_selected
              ? getSelectValue(optTaxList, parseInt(v.tax_selected))
              : "";
            v["qty"] = v.qty != "" ? v.qty : "";
            v["amount"] = v.amount != "" ? v.amount : "";
            v["gst"] = v.igst != "" ? v.igst : 0;
        ...
        console.log("initRowData", initRowData, grand_total);
        this.setState(
          {
            isEditDataSet: true,
